...
export class AuthService {
  ...
  async signUp(createUserDto: CreateUserDto): Promise<UserDocument> {
    ...
    if (userExists) {
      throw new BadRequestException(
        ...
        ErrorCodesEnum.USER_ALREADY_EXISTS,
      );
    }
  ...
  async signIn(data: AuthRequestDto): Promise<AuthResponseDto> {
    ...
    if (!passwordMatches) {
      throw new BadRequestException(
        ErrorMessagesEnum.PASSWORD_INCORRECT,
        ErrorCodesEnum.PASSWORD_INCORRECT,
      );
    }

    if (!user.verified) {
      throw new BadRequestException(
        ErrorMessagesEnum.EMAIL_NOT_VERIFIED,
        ErrorCodesEnum.EMAIL_NOT_VERIFIED,
      );
    }

    if (!user.isActive) {
      throw new BadRequestException(
        ErrorMessagesEnum.USER_INACTIVE,
        ErrorCodesEnum.USER_INACTIVE,
      );
    }
  ...
  async refreshTokens(authId: string, refreshToken: string) {
    ...
    if (!auth || !auth.refreshToken)
      throw new ForbiddenException(
        ErrorMessagesEnum.ACCESS_DENIED,
  ...
  async verify(code: string) {
    const user: UserDocument = await this.usersService.findByVerificationCode(
      code,
    );
